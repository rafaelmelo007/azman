...
function buildScript(osType: string | undefined, fqdn: string, server?: string): string {
...
function printVmRunCommandHelp(errorText: string, vm: VmInfo): void {
  ...
  if (text.includes("authorizationfailed") || text.includes("does not have authorization")) {
    ...
    console.log(
      chalk.yellow(
        "Required permission: Microsoft.Compute/virtualMachines/runCommand/action",
      ),
    );
...
export async function testDnsOnVm(input: TestDnsOnVmInput): Promise<void> {
