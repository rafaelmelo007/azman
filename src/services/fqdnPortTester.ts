...
type TestFqdnPortsInput = {
  fqdn: string;
  ports: number[];
};

async function testTcpPort(host: string, port: number, timeoutMs = 3000): Promise<boolean> {
...
export async function testFqdnPorts(input: TestFqdnPortsInput): Promise<void> {
  ...
  const checks = await Promise.all(
    ports.map(async (port) => ({ port, ok: await testTcpPort(fqdn, port) })),
  );
  ...
  const statusCells = checks
    .map((check) =>
      check.ok ? chalk.green("OPEN".padEnd(`:${check.port}`.length)) : chalk.red("CLOSED".padEnd(`:${check.port}`.length)),
    )
    .join(" | ");
