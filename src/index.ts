...
import {
  addVmHostOverride,
  listVmHostOverrides,
  removeVmHostOverride,
} from "./services/vmHostsInspector.js";
import {
  inspectVmWindowsServices,
  startVmWindowsService,
  stopVmWindowsService,
} from "./services/vmWindowsServicesInspector.js";
import {
  getAccount,
  isAzureCliInstalled,
  listSubscriptions,
  ...
  setSubscription,
  type AzSubscription,
} from "./utils/azRunner.js";

function normalizeForMatch(text: string): string {
  return text.trim().toLowerCase();
}

function formatAccountLine(account: { name?: string; id?: string; user?: { name?: string } }) {
...
function buildPromptLabel(subscriptionName?: string): string {
  if (!subscriptionName) return "AzMan>";
...
type HistoryRecorder = {
  start: (command: string, subscription?: string) => HistoryCapture;
  finish: (capture: HistoryCapture) => Promise<void>;
  restore: () => void;
};
...
function createHistoryRecorder(filePath: string): HistoryRecorder {
  const original = {
    log: console.log,
    info: console.info,
    warn: console.warn,
  ...
  let activeCapture: HistoryCapture | null = null;

  const tap =
    <T extends (...args: any[]) => void>(method: T) =>
    (...args: Parameters<T>) => {
      method(...args);
  ...
  console.log = tap(original.log) as typeof console.log;
  console.info = tap(original.info) as typeof console.info;
  console.warn = tap(original.warn) as typeof console.warn;
  console.error = tap(original.error) as typeof console.error;

  return {
    start: (command: string, subscription?: string) => {
    ...
    finish: async (capture: HistoryCapture) => {
...
function findSubscriptionMatches(subscriptions: AzSubscription[], target: string): AzSubscription[] {
  ...
  return subscriptions.filter(
    (s) => normalizeForMatch(s.name).includes(q) || normalizeForMatch(s.id).includes(q),
  );
}

async function showSubscriptions(): Promise<void> {
...
async function switchSubscription(target: string): Promise<void> {
  const subs = await listSubscriptions();
...
async function ensureAzureSession(): Promise<void> {
...
async function readCommandLine(promptLabel: string, history: string[]): Promise<{ text: string; history: string[] }> {
  ...
  try {
    while (true) {
      ...
      try {
        ...
        if (cmd.explanation) console.log(chalk.gray(cmd.explanation));
