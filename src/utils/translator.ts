import { z } from "zod";
import { TRANSLATION_RULES, type MatchContext } from "./translationRules.js";
...
export type TranslatorOutput = z.infer<typeof OutputSchema>;

export async function translateToAzCommand(userText: string): Promise<TranslatorOutput> {
...
function scoreRule(ctx: MatchContext, words: string[]): number {
  return words.reduce((score, word) => {
    const normalizedWord = word.trim().toLowerCase();
    if (!normalizedWord) return score;
...
async function callAI(user: string): Promise<string> {
  ...
  return JSON.stringify({
    kind: "clarify",
    message:
      'Try: "list vm", "list resources", "test dns <fqdn>", "subscriptions", or "switch subscription <name>".',
  });
}
