...
function wantsRunningOnly(normalizedText) {
    if (/\bnot\s+running\b/i.test(normalizedText))
        return false;
    return /\bonly\s+running\b|\brunning\s+only\b|\brunning\s+services?\b|\srunning$/i.test(normalizedText);
}
function hasExplicitStateFilter(normalizedText) {
    return /\bnot\s+running\b|\bonly\s+running\b|\brunning\s+only\b|\brunning\s+services?\b|\srunning$/i.test(normalizedText);
}
export const TRANSLATION_RULES = [
    ...
    },
    {
        ...
        output: ({ normalizedText, wantsJson }) => {
            const isShortcut = normalizedText === "public ips" ||
    ...
    },
    {
        ...
        minScore: 2,
        output: ({ normalizedText, wantsJson }) => {
            const isShortcut = normalizedText === "real ips" ||
    ...
    },
    {
        ...
        output: ({ normalizedText }) => {
            ...
            if (ports.length === 0 || hasInvalidPort) {
                return {
                    kind: "clarify",
                    message: "Ports must be numbers between 1 and 65535. Example: test public ips ports 80 443",
                };
            }
            return {
    ...
    },
    {
        ...
        output: ({ normalizedText }) => {
            ...
            if (!fqdn) {
                return {
                    kind: "clarify",
                    message: "Try: test dns <fqdn> | test dns <fqdn> server <dns-ip> | test dns <fqdn> trace",
                };
            }
            return {
                kind: "test_dns",
    ...
    },
    {
        ...
        minScore: 2,
        output: ({ normalizedText }) => {
            const lead = normalizedText.match(/^(?:test|check|diagnose)\s+dns\s+(?:on|from)\s+vm\s+(\S+)\s+(?:for\s+)?(.+)$/i);
            if (!lead)
                return null;
            ...
            if (!vmName || !fqdn) {
                return {
                    kind: "clarify",
                    message: "Try: test dns on vm <vm-name> <fqdn> | test dns on vm <vm-name> <fqdn> server <dns>",
                };
            }
            return {
                kind: "test_dns_vm",
    ...
    },
    {
        ...
        minScore: 2,
        output: ({ normalizedText }) => {
            const match = normalizedText.match(/^(?:diagnose|analyze|check)\s+dns(?:\s+drift)?\s+(\S+)\s+vm\s+(\S+)(?:\s+server\s+(\S+))?$/i);
            if (!match)
                return null;
            return {
    ...
    },
    {
        ...
        minScore: 1,
        output: ({ normalizedText }) => {
            const withPattern = normalizedText.match(/^(?:list|show|check)\s+(?:host|hosts)\s+(?:overrides?|entries)\s+(?:on|in)\s+vm\s+(\S+)\s+for\s+(.+)$/i) ||
                normalizedText.match(/^(?:list|show|check)\s+(?:host|hosts)\s+(?:overrides?|entries)\s+vm\s+(\S+)\s+for\s+(.+)$/i);
            if (withPattern) {
                return {
            ...
            const basic = normalizedText.match(/^(?:list|show|check)\s+(?:host|hosts)\s+(?:overrides?|entries)\s+(?:on|in)\s+vm\s+(\S+)$/i) ||
                normalizedText.match(/^(?:list|show|check)\s+(?:host|hosts)\s+(?:overrides?|entries)\s+vm\s+(\S+)$/i);
            if (!basic)
                return null;
            return {
                kind: "list_vm_host_overrides",
    ...
    },
    {
        ...
        minScore: 2,
        output: ({ normalizedText }) => {
            const vmThenEntry = normalizedText.match(/^(?:add|set|create)\s+(?:host|hosts)\s+(?:override|entry)?\s*(?:on|in)\s+vm\s+(\S+)\s+(\S+)\s+(\S+)$/i);
            if (vmThenEntry) {
            ...
            const entryThenVm = normalizedText.match(/^(?:add|set|create)\s+(?:host|hosts)\s+(?:override|entry)?\s+(\S+)\s+(\S+)\s+(?:on|in)\s+vm\s+(\S+)$/i);
            if (!entryThenVm)
                return null;
    ...
    },
    {
        ...
        minScore: 2,
        output: ({ normalizedText }) => {
            const vmThenHost = normalizedText.match(/^(?:remove|delete)\s+(?:host|hosts)\s+(?:override|entry)?\s*(?:on|in)\s+vm\s+(\S+)\s+(\S+)$/i);
            if (vmThenHost) {
            ...
            const hostThenVm = normalizedText.match(/^(?:remove|delete)\s+(?:host|hosts)\s+(?:override|entry)?\s+(\S+)\s+(?:on|in)\s+vm\s+(\S+)$/i);
            if (!hostThenVm)
                return null;
    ...
    },
    {
        ...
        minScore: 2,
        output: ({ normalizedText }) => {
            const serviceThenVm = normalizedText.match(/^(?:start|run)\s+(?:windows\s+)?services?\s+(.+?)\s+(?:on|in)\s+vm\s+(\S+)$/i);
            if (serviceThenVm) {
            ...
            const vmThenName = normalizedText.match(/^(?:start|run)\s+services?\s+(?:on|in)\s+vm\s+(\S+)\s+name\s+(.+)$/i);
            if (!vmThenName)
                return null;
    ...
    },
    {
        ...
        minScore: 2,
        output: ({ normalizedText }) => {
            const serviceThenVm = normalizedText.match(/^(?:stop)\s+(?:windows\s+)?services?\s+(.+?)\s+(?:on|in)\s+vm\s+(\S+)$/i);
            if (serviceThenVm) {
                const serviceName = serviceThenVm[1].trim();
                const vmName = serviceThenVm[2].trim();
                if (!vmName || !serviceName) {
            ...
            const vmThenName = normalizedText.match(/^(?:stop)\s+services?\s+(?:on|in)\s+vm\s+(\S+)\s+name\s+(.+)$/i);
            if (!vmThenName)
                return null;
            const vmName = vmThenName[1].trim();
            const serviceName = vmThenName[2].trim();
    ...
    },
    {
        ...
        output: ({ normalizedText }) => {
            ...
            const singleService = normalizedText.match(/^(?:check|show|list)\s+services?\s+(.+?)\s+(?:on|in)\s+vm\s+(\S+)(?:\s+(?:running|running\s+only|only\s+running|not\s+running))?$/i) ||
                normalizedText.match(/^(?:check|show|list)\s+service\s+(.+?)\s+vm\s+(\S+)$/i);
            if (singleService) {
            ...
            const byNames = normalizedText.match(/^(?:check|show|list)\s+services?\s+(?:on|in)\s+vm\s+(\S+)\s+names?\s+(.+)$/i) ||
                normalizedText.match(/^(?:check|show|list)\s+services?\s+vm\s+(\S+)\s+names?\s+(.+)$/i);
            if (byNames) {
            ...
            const byPattern = normalizedText.match(/^(?:check|show|list)\s+services?\s+(?:on|in)\s+vm\s+(\S+)\s+for\s+(.+)$/i) ||
                normalizedText.match(/^(?:check|show|list)\s+services?\s+vm\s+(\S+)\s+for\s+(.+)$/i);
            if (byPattern) {
                return {
                    kind: "inspect_vm_windows_services",
            ...
            const basic = normalizedText.match(/^(?:check|show|list)\s+services?\s+(?:on|in)\s+vm\s+(\S+)$/i) ||
                normalizedText.match(/^(?:check|show|list)\s+services?\s+vm\s+(\S+)$/i);
            if (!basic)
                return null;
            return {
                kind: "inspect_vm_windows_services",
    ...
    },
    {
        ...
        minScore: 1,
        output: ({ normalizedText }) => {
            const match = normalizedText.match(/^(?:test|check)\s+(?:dns\s+)?(?:reverse|ptr)\s+(\S+)(?:\s+server\s+(\S+))?$/i) ||
    ...
    },
    {
        ...
        minScore: 2,
        output: ({ normalizedText }) => {
            const match = normalizedText.match(/^(?:test|check)\s+dns\s+record\s+(\S+)\s+(a|aaaa|cname|mx|txt|ns|srv)(?:\s+server\s+(\S+))?$/i);
            if (!match)
                return null;
    ...
    },
    {
        ...
        minScore: 2,
        output: ({ normalizedText }) => {
            const match = normalizedText.match(/^(?:test|check)\s+(?:service|fqdn)\s+(\S+)(?:\s+ports?\s+(.+))?$/i) ||
            ...
            if (!fqdn || ports.length === 0 || hasInvalidPort) {
                return {
                    kind: "clarify",
                    message: "Try: test service <fqdn> ports 443 8443 (ports must be 1-65535)",
                };
            }
            return {
    ...
    },
    {
        ...
        minScore: 1,
        output: ({ normalizedText }) => {
            if (normalizedText === "subscriptions" ||
    ...
    },
    {
        ...
        minScore: 1,
        output: ({ normalizedText }) => {
            const match = normalizedText.match(/^(?:switch|change|use|set)\s+subscription\s+(.+)$/i) ||
    ...
    },
    {
        ...
        minScore: 1,
        output: ({ normalizedText, wantsJson }) => {
            const isShowAccount = normalizedText === "show account" ||
