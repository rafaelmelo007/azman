...
async function getAssignedPublicIps() {
    const result = await execa("az", ["network", "public-ip", "list", "--query", "[?ipAddress!=null].ipAddress", "-o", "tsv"], { stdio: ["ignore", "pipe", "pipe"] });
    return result.stdout
        .split("\n")
        .map((line) => line.trim())
        .filter(Boolean);
}
async function testTcpPort(ip, port, timeoutMs = 3000) {
...
export async function testPublicIps(portsInput = [443]) {
    ...
    const statusByIp = [];
    for (const ip of ips) {
        const checks = await Promise.all(ports.map(async (port) => ({ port, ok: await testTcpPort(ip, port) })));
        statusByIp.push({
            ip,
