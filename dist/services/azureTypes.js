...
export async function getPresentResourceTypes() {
...
function resolveType(userTypeRaw, presentTypes) {
    ...
    if (aliasHit && presentTypes.includes(aliasHit))
        return aliasHit;
...
async function execListByType(resolvedType, format) {
    await execa("az", ["resource", "list", "--resource-type", resolvedType, "-o", format], {
        stdio: "inherit",
    });
}
export async function handleListType(requestedType, format) {
    ...
    await execListByType(resolved, format);
}
