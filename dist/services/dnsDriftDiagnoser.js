...
async function getExternalARecords(fqdn) {
...
function buildVmScript(osType, fqdn, server) {
...
function printPermissionHint(errorText) {
...
async function getVmARecords(vm, fqdn, server) {
...
function printDiagnosis(fqdn, external, vmValues) {
    ...
    if (overlap.length === 0) {
        ...
        if (vmPrivate) {
            console.log(chalk.yellow("VM resolved to private IP(s) while external resolved to different/public IP(s). Likely hosts override or internal DNS split-horizon."));
        }
...
export async function diagnoseDnsDrift(input) {
