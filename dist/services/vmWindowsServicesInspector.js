...
function printErrorHelp(errorText) {
    ...
    if (text.includes("authorizationfailed") || text.includes("does not have authorization")) {
        ...
        console.log(chalk.yellow('Need action: "Microsoft.Compute/virtualMachines/runCommand/action" (e.g., Virtual Machine Contributor).'));
...
function truncateDebug(text, limit = 2000) {
    if (!text)
        return "<empty>";
...
function printDebugBlock(title, content) {
    console.log(chalk.magenta(`[debug] ${title}`));
    console.log(chalk.gray(content));
}
function buildPowerShellScript(input, includeProbe = false) {
    ...
    if (includeProbe)
        statements.push("Write-Output 'AZMAN_PROBE=1'");
    statements.push("$services = Get-Service");
    if (names.length > 0) {
        const quoted = names.map((n) => `'${n}'`).join(",");
        statements.push(`$names = @(${quoted})`);
        statements.push("$services = $services | Where-Object { $names -contains $_.Name -or $names -contains $_.DisplayName }");
    }
    else if (pattern) {
        statements.push(`$services = $services | Where-Object { $_.Name -match '${pattern}' -or $_.DisplayName -match '${pattern}' }`);
    }
    if (input.runningOnly !== undefined) {
        statements.push(input.runningOnly
    ...
    statements.push("if ($rows.Count -gt 0) { ($rows | Format-Table -AutoSize | Out-String -Width 4096).TrimEnd() | Write-Output }");
    return statements.join("; ");
}
function buildStartServiceScript(serviceName) {
...
function buildStopServiceScript(serviceName) {
    const safeServiceName = serviceName.replace(/'/g, "''").trim();
    return [
        "$ErrorActionPreference = 'Stop'",
        `$target = '${safeServiceName}'`,
...
function parseCountFromMessage(message) {
...
function stripCountLines(message) {
    return message
        .split(/\r?\n/)
...
function estimateRowsFromTable(tableOutput) {
    return tableOutput
        .split(/\r?\n/)
        .map((line) => line.trim())
        .filter(Boolean).length;
}
function formatFilterLabel(input) {
    const parts = [];
    if (input.names && input.names.length > 0)
        parts.push(`names=${input.names.join(",")}`);
    if (input.pattern)
        parts.push(`pattern=${input.pattern}`);
    if (input.runningOnly !== undefined)
        parts.push(`state=${input.runningOnly ? "running" : "not-running"}`);
    return `Filter: ${parts.join(" | ")}`;
}
async function runVmScript(vm, script) {
    const out = await execa("az", [
        "vm",
        "run-command",
        "invoke",
...
export async function inspectVmWindowsServices(input) {
    ...
    try {
        ...
        if (debugEnabled) {
            ...
            printDebugBlock("az vm run-command summary", `stdout.length=${run.stdout.length}\nstderr.length=${run.stderr.length}\nvalueItems=${parsed?.value?.length ?? 0}\ncodes=${codes || "<none>"}`);
        ...
        console.log("");
        if (!message.trim()) {
            console.log(chalk.yellow("Azure run-command returned empty stdout for this VM. Unable to read services output.\n"));
            return;
        }
        ...
        if (count === 0 || (!hasRows && count === null)) {
            const noRowsMessage = input.runningOnly === true
...
export async function startVmWindowsService(input) {
    const allVms = await listVms();
    const matches = findVmMatches(allVms, input.vmName);
    const vm = await selectVm(matches, input.vmName);
    if (!vm) {
    ...
    try {
        ...
        if (status === "STARTED") {
            ...
            if (normalize(currentStatus) === "running") {
                ...
                console.log(chalk.yellow(`Start command executed for "${name ?? input.serviceName}", but current status is "${currentStatus || "unknown"}".\n`));
            }
            return;
        }
...
export async function stopVmWindowsService(input) {
    const allVms = await listVms();
    const matches = findVmMatches(allVms, input.vmName);
    const vm = await selectVm(matches, input.vmName);
    if (!vm) {
    ...
    try {
        ...
        if (status === "STOPPED") {
            ...
            if (normalize(currentStatus) === "stopped") {
                ...
                console.log(chalk.yellow(`Stop command executed for "${name ?? input.serviceName}", but current status is "${currentStatus || "unknown"}".\n`));
            }
            return;
        }
