...
function buildScript(osType, pattern) {
...
function buildAddHostScript(osType, ip, hostname) {
...
function buildRemoveHostScript(osType, hostname) {
    const safeHost = hostname.replace(/'/g, "''").trim();
    const isWindows = normalize(osType ?? "") === "windows";
    if (isWindows) {
        return [
...
function parseHostsActionResult(message) {
    const line = message
        .split(/\r?\n/)
        .map((l) => l.trim())
...
export async function listVmHostOverrides(input) {
    const allVms = await listVms();
...
export async function addVmHostOverride(input) {
...
export async function removeVmHostOverride(input) {
    const vm = await resolveVm(input.vmName);
    if (!vm) {
        console.log(chalk.yellow(`No VM matched "${input.vmName}".`));
        return;
